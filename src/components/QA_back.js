...
export default function QA() {
	...
	const slidesIndexes = slides.map( slide => slide.index)
	const indexMax = Math.max(...slidesIndexes)
	...
	function handlePrevSlide() {
		...
		if (newIndex < 0) {
			const doubleSlides = [...slides, ...slides]
			console.log(doubleSlides);
			setSlides(doubleSlides)
			// 複製整個array，並將unshift 到原始array的前面
			setActiveSlide(slides[indexMax])
		} else {
